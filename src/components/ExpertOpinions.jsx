...
const parseBreakdown = (text = "") => text.split("\n").map((line) => line.trim()).filter(Boolean).map((line) => { const [label, ...valueParts] = line.split(":"); return { label: label.trim(), value: valueParts.join(":").trim() }; }).filter((item) => item.label && item.value);
...
export default function ExpertOpinions({ experts, setExperts, onSource, onAudit, getDocumentMeta }) {
  ...
  const saveExpert = (values) => {
    ...
    const previous = { date: editing.date, name: editing.name, role: editing.role, disability: editing.disability, opinion: editing.opinion, full: editing.full, breakdown: breakdownText(editing), sourceTitle: editing.source?.title, sourceDate: editing.source?.date, sourceContent: editing.source?.content };
    const changedFields = {};
    ...
    setExperts((items) => items.map((item) => item.id === editing.id ? {
      ...item, originalValues: item.originalValues || previous, date: values.date, name: values.name, role: values.role, disability: values.disability, opinion: values.opinion, full: values.full,
      breakdown: parseBreakdown(values.breakdown), manuallyEdited: true, editedFields: { ...(item.editedFields || {}), ...changedFields }, editMetadata: { editedBy: "מיישב תביעה", editedAt: timestamp },
  ...
  const openExpert = (expert) => onSource(expert.source, {
    ...
    aiSummary: expert.opinion, fullSummary: expert.full, breakdown: expert.breakdown, totalDisability: expert.disability,
    ...
    <section className="panel compact-timeline-panel">
      ...
      <div className="unified-timeline expert-timeline">
        {experts.map((expert) => {
          ...
          return <TimelineRow key={expert.id} date={expert.date} title={`${expert.name} – ${expert.role}`} disability={expert.disability || "לא נמצא במסמכים"} summary={expert.opinion} editMetadata={expert.editMetadata} readability={meta.extractionConfidence} reviewed={meta.manuallyReviewed} onOpen={() => openExpert(expert)} actions={<>
      ...
      {editing && <EditModal title="עריכת חוות דעת מומחה" fields={fields} editedFields={editing.editedFields || {}} initialValues={{ date: editing.date, name: editing.name, role: editing.role, disability: editing.disability || "", opinion: editing.opinion || "", full: editing.full || editing.opinion || "", breakdown: breakdownText(editing), sourceTitle: editing.source?.title || "", sourceDate: editing.source?.date || "", sourceType: editing.source?.type || "", sourceContent: editing.source?.content || "" }} onCancel={() => setEditing(null)} onSave={saveExpert} />}
      {adding && <DocumentUploadForm kind="expert" title="הוספת חוות דעת" section="חוות דעת מומחים" fields={fields} initialValues={{ date: "", name: "", role: "", disability: "", opinion: "", full: "", breakdown: "", sourceTitle: "", sourceDate: "", sourceType: "", sourceContent: "" }} onAudit={onAudit} onCancel={() => setAdding(false)} onSave={(values) => {
        const timestamp = new Date().toLocaleString("he-IL");
        const next = { id: `expert-${Date.now()}`, date: values.date, name: values.name, role: values.role, disability: values.disability, opinion: values.opinion, full: values.full, breakdown: parseBreakdown(values.breakdown), manuallyEdited: true, editMetadata: { editedBy: "מיישב תביעה", editedAt: timestamp }, source: { title: values.sourceTitle, date: values.sourceDate, type: values.sourceType || "חוות דעת", content: values.sourceContent, aiSummary: values.opinion, fullSummary: values.full, sourcePreviewUrl: values.sourcePreviewUrl, sourceFileType: values.sourceFileType } };
